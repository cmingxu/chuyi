...
document.addEventListener("DOMContentLoaded", () => {
  console.log("dom content loaded in renderer process");
  if(window.location.host == JinRiTeMaiConsts.host) {
    let toolbar = new Toolbar(document, window);
    toolbar.show();
    toolbar.registerEventListeners();
  }else{
      console.log("in chat panel");
      setTimeout(() => {
        (new ChatWindow()).onLoaded();
      }, 4000);
  }
});
class ChatWindow{
